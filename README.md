...
A simple Java web application (Servlet/JSP) for managing polls, options and voters.

## Overview

This project provides a basic online voting system implemented with Java Servlets, JSP views, and DAOs for data access. It is structured for deployment as a WAR to a servlet container (for example, Apache Tomcat).

## Project structure

- `src/main/java` - Java source code
  - `Authentication` - authentication logic
  - `Controllers` - servlet controllers (AdminController, PollController, VoterController, etc.)
  - `DAO` - data access objects (OptionDAO, PollDAO, VoteCountDAO, VoterDAO)
  - `Models` - domain models (Poll, PollOption, Voter, VoteCount)
- `src/main/webapp` - JSP views and web configuration
  - `WEB-INF` - `web.xml`, Spring/dispatcher config and view JSPs

## Prerequisites

- JDK 8 or newer
- Apache Maven
- Servlet container (Apache Tomcat) to run the packaged WAR
...
2. Deploy the generated WAR file from `target/` to your servlet container (e.g., Tomcat `webapps/`) or run from your IDE.

## Notes

- Database configuration and datasource setup depends on your environment. Inspect the DAOs and web configuration files (`src/main/webapp/WEB-INF`) to configure connections.
- Views are implemented with JSP under `src/main/webapp/WEB-INF/views`.

## Contributing

Feel free to open issues or pull requests for bug fixes and improvements.
